module parameters_and_co
  ...
  integer :: Nread, Ndim, tNdim, Nbcoef, Typecontfrac, typePairpot, nbp
  integer :: n1min, n1max, n2min, n2max, n3min, n3max
  integer :: lattype, segment, type_extra

  real(kind=8) :: tmp, mmass, Ar, rho, Br, dE, cutoff, extra_coef
...
contains
  ...
  !!*** 
    ...
    open(unit=25,file="input_pairpot_rdynmat.in")
    
    read(25,*) lattype
    
!    if (lattype.eq.2) then
    	read(25,*) a1(:)
    	read(25,*) a2(:)
    	read(25,*) a3(:)
	...
	write(*,*) "Building lattice"
	write(*,*) " "
!    else
	if (lattype.eq.1) then
	   open(unit=26,file="atomiccoord.xyz")
	   read(26,*) Ndim
           close(26)
!	else
!		write(*,*) "Unknown type of lattice!"
!		stop
	endif 
!    endif
    ...
    read(25,*) mmass
    read(25,*) typePairpot
    if (typePairpot.eq.1) then
	read(25,*) Ar, Br
    else
	if (typePairpot.eq.2) then
		read(25,*) Ar, Br, cutoff
        else
        	if (typePairpot.eq.3) then
			read(25,*) Ar, rho, Br
		else
			write(*,*) "Unknown type of pair potential!"
			stop
		endif
	endif
    endif
    ...
    ! echo variables
    write (*,*) "Input variables read as"
    write (*,*) " a1 - prim vec = ", a1
    write (*,*) " a2 - prim vec = ", a2
    write (*,*) " a3 - prim vec = ", a3
    write (*,*) " Lattice sites: "
    write (*,*) " direc 1 : from - to : ", n1min, n1max
    write (*,*) " direc 2 : from - to : ", n2min, n2max
    write (*,*) " direc 3 : from - to : ", n3min, n3max
    write (*,*) "  "
    write (*,*) "  Mass = ", mmass
    write (*,*) "  "
    if (typePairpot.eq.1) then
	write (*,*) " Pair potential: LJ"
	write (*,*) " parameters Ar = ", Ar, " Br = ", Br
    else
	write (*,*) " Pair potential: exponential + 1/x^6"
	write (*,*) " parameters Ar = ", Ar, "rho = ", rho, " Br = ", Br
    endif
